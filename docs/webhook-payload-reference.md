...
- **`data.sourceEvent`** — the source event type that triggered the export (one of the
  [supported events](nexcore-prerequisites.md)). Worth using: it tells you what kind of
  entity `entityId` refers to. For `rental.order.completed` and `rental.order.archived`,
  `entityId` is the **order's** id, so you can call `GET /api/v1/order/{entityId}` for its
  details — this app uses that to name files after the order number (see below). The
  `rental.resourceAssignment.*` events carry a ResourceAssignment id instead.
...
5. For `rental.order.completed`, look the order up via `GET /api/v1/order/{entityId}` and
   name the file `<number>_<YYYYMMDDTHHMMSS>.<ext>` instead. Controlled by
   `ORDER_NUMBER_FILENAMES` (default on); any failure falls back to step 4's naming. The
   lookup happens after the `200`, so it does not affect the response time.
