...
### Naming order exports after the order number

A UUID filename is safe but unreadable. For exports triggered by
`rental.order.completed`, the service can name the PDF after the order's number instead:

```
OUTPUT_DIR/<tenantId>/<YYYY-MM-DD>/<orderNumber>_<YYYYMMDDTHHMMSS>.pdf
```

This works because the export webhook tells you *which* event triggered it
(`data.sourceEvent`) and, for that event, `entityId` **is** the order's id. The service
therefore does one extra call to `GET /api/v1/order/{entityId}` — the same API key as the
subscription endpoints, no separate auth — and reads the `number` field.

The timestamp is always appended: an order number comes from the partner system and is not
unique over time, so re-completing the same order would otherwise overwrite the earlier
export.

Controlled by `ORDER_NUMBER_FILENAMES` (default `true`). It degrades gracefully — no
credentials, a failed lookup, or an order without a number all fall back to the
`<entityId>_<index>` naming, and the file is still stored. The lookup runs *after* the
`200` has been sent, so it never slows the webhook response down. See
[`app/naming.py`](app/naming.py).

**Still seeing `<entityId>_0.pdf`?** The fallback is deliberately quiet about *why*, so
check the service log — it always says which of these it was:

| Log line | Meaning |
|---|---|
| `no nexcore API credentials — naming event … from entityId` | `NEXCORE_BASE_URL` / `NEXCORE_API_KEY` / `NEXCORE_API_KEY_ID` are not all set. Logged at `DEBUG`, so set `LOG_LEVEL=DEBUG` to see it. |
| `order lookup failed for … — naming event … from entityId instead` | The API call failed — wrong base URL, invalid or revoked key, or the instance was unreachable. |
| `order … not found — naming event … from entityId instead` | nexcore returned `404` for that id. The key may belong to a different tenant than the webhook. |
| `order … has no number — naming event … from entityId instead` | The order exists but its `number` field is empty. Nothing to name the file after. |

No log line at all means the lookup was never attempted: either `ORDER_NUMBER_FILENAMES` is
`false`, or the event was not `rental.order.completed` — check `data.sourceEvent` in the
payload, and see the note below.

> Only `rental.order.completed` is handled. The `rental.resourceAssignment.*` events carry a
> ResourceAssignment id rather than an order id, so they keep the `entityId` naming.
...
> Set `ORDER_NUMBER_FILENAMES=false` for the self-test. The sample webhook uses
> `rental.order.completed`, so with `.env.example`'s placeholder credentials the service
> would try one order lookup, have it rejected, and log a warning before falling back to
> the `entityId` naming. The download itself is unaffected either way.
...
| Doc | What |
|---|---|
| [nexcore-prerequisites.md](docs/nexcore-prerequisites.md) | The 4 nexcore-side preconditions + the supported events |
