...
class _SubscriptionHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        type(self).requests.append({"headers": dict(self.headers), "path": self.path})
        prefix = "/api/v1/subscription/search/"
        order_prefix = "/api/v1/order/"
        if self.path.startswith(prefix):
            ext = self.path[len(prefix):]
            sub = type(self).store.get(ext)
            self._send_json(200, sub) if sub else self._send_json(404, None)
        elif self.path.startswith(order_prefix):
            order = type(self).orders.get(self.path[len(order_prefix):])
            self._send_json(200, order) if order else self._send_json(404, None)
        else:
...
def test_get_order_returns_the_order(server: str) -> None:
    _SubscriptionHandler.orders["ord-1"] = {"id": "ord-1", "number": "4711"}
    client = make_client(server)

    order = client.get_order("ord-1")

    assert order is not None
    assert order["number"] == "4711"


def test_get_order_returns_none_on_404(server: str) -> None:
    client = make_client(server)

    assert client.get_order("does-not-exist") is None


def test_get_order_sends_api_key_headers(server: str) -> None:
    _SubscriptionHandler.orders["ord-1"] = {"id": "ord-1", "number": "4711"}
    client = make_client(server)
    ...
    assert headers[API_KEY_HEADER.lower()] == "api-key-data"
    assert headers[API_KEY_ID_HEADER.lower()] == "api-key-id"
    assert req["path"] == "/api/v1/order/ord-1"
