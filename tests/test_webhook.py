...
def test_healthz(tmp_path: Path) -> None:
    ...
    assert body["version"]
    # The config summary exists so an install that loaded no .env is visible here
    # instead of looking healthy; the test client runs without one.
    assert body["configured"] is False
    assert "PUBLIC_WEBHOOK_URL" in body["missing"]
