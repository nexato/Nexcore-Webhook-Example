...
class _OrderApiHandler(BaseHTTPRequestHandler):
    """Minimal stand-in for GET /api/v1/order/{id}."""
    ...
    def do_GET(self) -> None:  # noqa: N802
        prefix = "/api/v1/order/"
        order = type(self).orders.get(self.path[len(prefix):]) if (
            self.path.startswith(prefix)
        ) else None
        body = json.dumps(order).encode() if order else b""
...
@pytest.fixture
def order_api():
    _OrderApiHandler.orders = {}
    server = _FastHTTPServer(("127.0.0.1", 0), _OrderApiHandler)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    try:
        yield f"http://127.0.0.1:{server.server_address[1]}"
...
def test_build_target_path_uses_basename_without_index_for_a_single_file() -> None:
    p = build_target_path(
        "/out", "tenantA", "entity-123", 0, "application/pdf", basename="4711_20260831T142530"
    )
    today = date.today().isoformat()
    assert p == Path("/out/tenantA") / today / "4711_20260831T142530.pdf"


def test_build_target_path_appends_index_to_basename_for_multiple_files() -> None:
    today = date.today().isoformat()
    for index in (0, 1):
        p = build_target_path(
            "/out", "tenantA", "entity-123", index, "application/pdf", basename="4711_ts", total=2
        )
        assert p == Path("/out/tenantA") / today / f"4711_ts_{index}.pdf"


def test_build_target_path_sanitizes_the_basename(tmp_path: Path) -> None:
    out = tmp_path / "output"
    for basename in ["../../../../tmp/evil", "/tmp/evil", "..", "A/B"]:
        dest = build_target_path(out, "tenant", "entity", 0, "application/pdf", basename=basename)
...
def test_download_files_names_a_single_file_from_the_basename(
    file_server: str, tmp_path: Path
) -> None:
    out = tmp_path / "output"
    ...
    paths = download_files(
        ...
        entity_id="ent-1",
        timeout=10,
        basename="4711_20260831T142530",
    )

    today = date.today().isoformat()
    assert paths == [out / "tenantA" / today / "4711_20260831T142530.pdf"]
    assert paths[0].read_bytes() == PDF_BYTES


def test_download_files_disambiguates_multiple_files_under_one_basename(
    file_server: str, tmp_path: Path
) -> None:
    out = tmp_path / "output"
    files = [
    ...
    today = date.today().isoformat()
    assert paths == [
        out / "tenantA" / today / "4711_ts_0.pdf",
        out / "tenantA" / today / "4711_ts_1.zip",
    ]
...
def _post_order_export(
    file_server: str, order_api: str | None, tmp_path: Path, entity_id: str = "order-777"
):
    """Send a signed rental.order.completed export webhook; return (settings, output dir)."""
    ext_id, secret = "e2e-ext", "e2e-secret"
    db = tmp_path / "state.sqlite"
    out = tmp_path / "output"
    settings = Settings(
        ...
        output_dir=out,
        nexcore_base_url=order_api or "",
        nexcore_api_key="key-data" if order_api else "",
        nexcore_api_key_id="key-id" if order_api else "",
    )
    store = Store(db)
    store.save_subscription(ext_id, "sub-uuid", secret)
    main.app.dependency_overrides[main.get_settings] = lambda: settings
    try:
        ...
        response = TestClient(main.app).post("/webhook", content=body, headers=headers)
        assert response.status_code == 200 and response.json()["status"] == "accepted"
    finally:
        main.app.dependency_overrides.clear()
    return out / "tenant-9" / date.today().isoformat()


def test_webhook_names_an_order_export_from_the_order_number(
    file_server: str, order_api: str, tmp_path: Path
) -> None:
    _OrderApiHandler.orders["order-777"] = {"id": "order-777", "number": "4711"}

    target_dir = _post_order_export(file_server, order_api, tmp_path)

    stored = list(target_dir.iterdir())
    assert len(stored) == 1
    assert re.fullmatch(r"4711_\d{8}T\d{6}\.pdf", stored[0].name), stored[0].name
    assert stored[0].read_bytes() == PDF_BYTES


def test_webhook_falls_back_to_entity_id_when_the_order_is_unknown(
    file_server: str, order_api: str, tmp_path: Path
) -> None:
    # order_api serves no orders → 404 on the lookup
    target_dir = _post_order_export(file_server, order_api, tmp_path)

    assert [p.name for p in target_dir.iterdir()] == ["order-777_0.pdf"]


def test_webhook_falls_back_to_entity_id_without_credentials(
    file_server: str, tmp_path: Path
) -> None:
    target_dir = _post_order_export(file_server, None, tmp_path)

    assert [p.name for p in target_dir.iterdir()] == ["order-777_0.pdf"]


def test_webhook_sanitizes_a_malicious_order_number(
    file_server: str, order_api: str, tmp_path: Path
) -> None:
    """An order number is partner-system free text — it must not escape OUTPUT_DIR.

    Guards the integration of the sanitizing driven in test_build_target_path_*.
    """
    _OrderApiHandler.orders["order-777"] = {"number": "../../../../etc/passwd"}
    out = tmp_path / "output"
    ...
    stored = list(target_dir.iterdir())
    assert len(stored) == 1
    assert stored[0].resolve().is_relative_to(out.resolve())
