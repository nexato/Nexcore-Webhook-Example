...
The same API key also unlocks the rest of ``/api/**``. ``get_order`` uses that to
resolve an order's ``number`` for file naming (see ``app/naming.py``):

- ``GET    /api/v1/order/{id}``               200 / 404 (``OrderRestController``)
...
class NexcoreClient:
    ...
    def get_order(self, order_id: str) -> dict[str, Any] | None:
        """Return the order for ``order_id`` (200) or ``None`` (404).

        Used to look up an order's ``number`` for file naming. Note the response is
        the full order entity; only ``number`` is of interest here.
        """
        with self._client() as client:
            resp = client.get(f"{ORDER_PATH}/{order_id}")
            if resp.status_code == 404:
                return None
            resp.raise_for_status()
            return resp.json()
