...
Callers may override the stem via ``basename`` — ``app/naming.py`` uses this to name
order exports after the order number. This module stays free of that domain knowledge;
it only sanitizes whatever stem it is handed.
...
def build_target_path(
    ...
    *,
    ...
    Without ``basename`` the file is named ``<entityId>_<index>.<ext>``.

    ``basename`` overrides the stem (see ``app/naming.py``, which derives it from the
    order number). It is sanitized like any other untrusted component; if nothing
    usable survives, we fall back to the ``entityId`` scheme. The ``_<index>`` suffix
    is then only appended when the event actually carries more than one file
    (``total > 1``) — a single file keeps the clean ``<basename>.<ext>``.

    ``tenant_id``, ``entity_id`` and ``basename`` are all sanitized, and the resolved
    result is asserted to stay within ``output_dir`` (raises ``DownloadError``
    otherwise).
    """
    tenant = safe_path_component(tenant_id, "unknown-tenant")
    ext = extension_for_mime(mime_type)
    stem = safe_path_component(basename, "") if basename else ""
    if stem:
        name = f"{stem}{ext}" if total <= 1 else f"{stem}_{index}{ext}"
    else:
        name = f"{safe_path_component(entity_id, 'unknown')}_{index}{ext}"
    base = Path(output_dir)
    dest = base / tenant / date.today().isoformat() / name
...
def download_files(
    ...
    *,
    ...
    ``basename`` overrides the filename stem (see ``build_target_path`` and
    ``app/naming.py``); without it files are named from ``entity_id``.
