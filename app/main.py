...
7. Respond **200 fast** and download the files in a background task (which also
   resolves the order-number filename for order exports).
...
from .config import ENV_FILE_VAR, Settings, env_file_candidates, resolve_env_file
from .downloader import download_files
from .models import WebhookEvent
from .naming import resolve_order_basename
...
def _log_config_source() -> None:
    """Say where configuration came from — the first line of any install triage.

    A missing ``.env`` is not an error (containers configure via real env vars),
    but it is the single most common reason a freshly installed service runs with
    nothing but defaults. Silence here means guessing later, so log both the file
    that was used and, when there was none, every path that was tried.
    """
    if _env_file is not None:
        logger.info("configuration loaded from %s", _env_file)
    else:
        logger.warning(
            "no .env file found (tried: %s) — using real environment variables and "
            "defaults only. Set %s to an absolute path if the working directory is "
            "not the project folder.",
            ", ".join(str(c) for c in env_file_candidates()),
            ENV_FILE_VAR,
        )
    missing = _settings.missing_required()
    if missing:
        logger.warning("configuration incomplete — unset: %s", ", ".join(missing))
...
def process_event(
    ...
    For order exports the filename is derived from the order's number via an extra
    REST lookup (``app/naming.py``). That happens here, after the 200 has already
    been sent, so it never adds to the webhook response time; any problem with it
    falls back to the default ``entityId`` naming.
    """
    ...
    basename = resolve_order_basename(event, settings)
    try:
...
app = FastAPI(title="Nexcore Webhook Example", version=__version__, lifespan=lifespan)


@app.get("/healthz")
def healthz(settings: SettingsDep) -> dict[str, object]:
    """Liveness **and** a configuration summary.

    Plain ``{"status": "ok"}`` made a service that had silently failed to load its
    ``.env`` look perfectly healthy. ``configured``/``missing`` name the env vars
    that are unset for the current mode; no values are returned, only names.
    """
    missing = settings.missing_required()
    return {
        "status": "ok",
        "version": __version__,
        "config_source": str(_env_file) if _env_file else "environment only",
